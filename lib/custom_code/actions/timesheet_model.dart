...
class TimesheetModel {
  ...
  static Future<dynamic> getLatestTimeIn(int userId) async {
    ...
    try {
      return await Repo.rawQuery('SELECT id, MAX(time_in) as time_in, time_out FROM timesheet WHERE user_id=? AND DATE(time_in) =?', [userId, dtStr]);
    } catch (err) {
      debugPrint('Repo Error: $err');
      throw Error();
    }
  ...
  static Future<dynamic> getLatestDetails(DateTime dtTime, int uId) async {
    ...
    try {
      return await Repo.rawQuery('SELECT id, MAX(time_in) as time_in, MAX(time_out) as time_out FROM timesheet WHERE user_id=? AND DATE(time_in) =?', [uId, dtStr]);
    } catch (err) {
      debugPrint('Repo Error: $err');
      throw Error();
    }
